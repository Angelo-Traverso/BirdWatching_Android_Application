...
class MyObservations : Fragment() {
    ...
    private fun populateUserObservation()
    {
        ...
        if (sqlDate  != null) {
            val userObservationEntry1 = UserObservation("1", "ST10081927", sqlDate , "Paco", Geocoder(requireContext()))
            val userObservationEntry2 = UserObservation("2", "ST10081928", sqlDate , "What", Geocoder(requireContext()))
            val userObservationEntry3 = UserObservation("3", "ST10081929", sqlDate , "Test", Geocoder(requireContext()))
    ...
    private fun addObservationViewToContainer(userObservation: UserObservation) {
        ...
        observationView.findViewById<TextView>(R.id.tvScientificName).text = userObservation.BirdName
        observationView.findViewById<TextView>(R.id.tvLocation).text = userObservation.Location.toString()
        observationView.findViewById<TextView>(R.id.tvDateSpotted).text = userObservation.Date.toString()
